...
import { shuffleArray } from "../../services/misc";
...
export default function PanelsView(props) {
    ...
    onMount(async () => {
        ...
        try {
            ...
            if (item().display_all_quotes === true) {
                const result3 = await pb.collection("quotes").getFullList();
                const randomQuotes = shuffleArray(result3);
                setQuotes(randomQuotes);
                import.meta.env.DEV && console.log("All quotes loaded", result3.length);
            }
        } catch (error) {
            setError(true);
                ...
                <div class="min-h-screen flex flex-col">
                    ...
                    <Show when={quotes().length > 0}>
                        <div class="flex items-center justify-center px-6"
                            classList={{
                                "flex-none": birthdays().length > 0 || news().length > 0,
                                "flex-1": birthdays().length === 0 && news().length === 0,
                                "text-[2em]": birthdays().length > 0 || news().length > 0,
                                "text-[3em]": birthdays().length === 0 && news().length === 0
                            }}> {/* Row 2 (optional) */}
                            <div class="text-center">
                                <span class="text-[0.65em] pr-2">
                                ...
                                <span>{quotes()[nextQuote()].content}</span>
                                <span class="text-[0.65em] italic ml-2">{quotes()[nextQuote()].author}</span>
                                <span class="text-[0.65em] pl-2">
                                    <svg style={`fill: ${item().font_color};`} class="inline-block" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M448 296c0 66.3-53.7 120-120 120h-8c-17.7 0-32-14.3-32-32s14.3-32 32-32h8c30.9 0 56-25.1 56-56v-8H320c-35.3 0-64-28.7-64-64V160c0-35.3 28.7-64 64-64h64c35.3 0 64 28.7 64 64v32 32 72zm-256 0c0 66.3-53.7 120-120 120H64c-17.7 0-32-14.3-32-32s14.3-32 32-32h8c30.9 0 56-25.1 56-56v-8H64c-35.3 0-64-28.7-64-64V160c0-35.3 28.7-64 64-64h64c35.3 0 64 28.7 64 64v32 32 72z" /></svg>
                                </span>
                            </div>
                        </div>
                    </Show>
                    <Show when={birthdays().length > 0 || news().length > 0}>
                        <div class="flex-1 flex flex-row"> {/* Row 3 (optional) */}
